function generateExcuse(category) {
  ...
  //
  const randomSubject = pickRandom(FRAGMENTS[category].subjects);
  const randomVerb = pickRandom(FRAGMENTS[category].verbs);
  const randomReason = pickRandom(FRAGMENTS[category].reasons);
  ...
  return `${randomSubject} ${randomVerb} ${randomReason}.`;
  // Your code here
}
